...
void positionCallback(const laserp::Point32Plus::ConstPtr &msg) {
	...
	if (gAnalyzer->isRunning()) {
		if (now - gAnalyzer->getStartTime() > AnalyzeTime) {
			...
			*((unsigned char *)p)     = 0xdd;
			*((unsigned char *)p + 1) = 0xbb;
			*((unsigned char *)p + 2) = 0xaa;
			*((unsigned char *)p + 3) = static_cast<unsigned char>(ins);
			*((unsigned char *)p + 4) = static_cast<unsigned char>(ins);
			*((unsigned char *)p + 5) = static_cast<unsigned char>(255 - ins);
			*((unsigned char *)p + 6) = static_cast<unsigned char>(255 - ins);
			*((unsigned char *)p + 7) = 0x0e;
			bbuf[64] = '\0';
			std::string sss(bbuf);
...
//	ROS_INFO("Serial port write laser position instruction [ %s ].", posIns.c_str());
	gSerialPort->writeBuffer(posIns);
}
